  ...
  const handleLikedFn = async () => {
    try {
      const response = await axios.put(
        "http://localhost:8080/api/post/like-dislike",
        {
          postId: postData?._id,
        },
        { withCredentials: true }
  ...
  const handleCommentBoxOpenClose = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/comment/get-post-comment/${postData?._id}`,
        {
          withCredentials: true,
        }
      );
  ...
  return (
    <Card padding={0}>
      ...
      <div className="text-md p-4 my-3 whitespace-pre-line grow">
        {seeMore
          ? desc
          : `${desc?.slice(0, 35)} ${desc?.length > 65 ? "..." : ""}`}{" "}
        {desc?.length < 65 ? null : (
          <span
          ...
          <div className="p-4 w-full">
            <div className="flex gap-2 items-center">
              ...
              >
                ...
                <button
                  type="submit"
                  className="cursor-pointer bg-blue-800 text-white rounded-3xl py-1 px-7"
                >
                  Send
                </button>
              </form>
